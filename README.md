...
4. [핵심 기능 — 생성 파이프라인](#4-핵심-기능--생성-파이프라인)
5. [데이터 모델 (일반화)](#5-데이터-모델-일반화)
6. [기술적 의사결정](#6-기술적-의사결정)
7. [트러블슈팅](#7-트러블슈팅)
8. [테스트 (pytest)](#8-테스트-pytest)
9. [성능 및 효율](#9-성능-및-효율)
10. [회고](#10-회고)
11. [API 설계 (예시)](#11-API-설계-예시)
...
핵심 로직과 트러블슈팅 과정에서 드러난 엣지케이스들을 pytest로 검증했습니다.

| 테스트 대상 | 검증 내용 | 관련 |
|---|---|---|
| AI 어댑터 재시도 | 타임아웃/일시적 오류 시 재시도 후 성공, 재시도 불가능한 에러는 즉시 실패, 지수 백오프 간격 | 비용 기반 재시도 범위 설계 |
| 원그리드 이미지 분할 | 그리드 크롭 경계 탐지 로직 | 그리드 컷 분할 경계가 어긋나던 문제 |
| 인증 보안 | 비밀번호 해싱, 존재하지 않는 계정에 대해서도 동일한 응답 시간 유지 (타이밍 어택 방지) | 로그인 응답 시간으로 계정 존재 여부가 추측되던 문제 |
| 생성 작업 복구 | 서버 재시작 후 멈춰버린 작업을 실패로 정리하는 복구 로직 | 배포로 서버 재시작 시 처리 중이던 작업이 방치되던 문제 |
| Export (PDF) | 정상 생성 플로우 및 실패 케이스 처리 | 스토리보드 PDF 생성 |
| 오브젝트 스토리지 | 업로드 · 다운로드 정상 동작 | R2 스토리지 매직바이트 검증 |
| 리소스 소유권 검증 | 타인 리소스 접근 시 404 처리 | 소유권 검사 = 404 |
